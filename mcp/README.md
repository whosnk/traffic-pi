...
这些工具向当前画布发送沙盒原生快捷键，保留地图、时间和进度。暂停/继续会将速度归一到 1 倍；加速依次为 1、5、30、3600 倍。午夜没有车时可加速推进时间，而不必重新加载。弹窗或非 Sandbox 页面可能拦截快捷键，工具返回的是发送确认，不是读取到的内部播放状态。
...
工具通过真实 MCP stdio 协议通知本地网页；网页重新加载官方 Montlake 工作日场景，从 08:00 自动播放，车辆是真实仿真生成。每次调用都会重置仿真进度。普通打开工作台也直接进入 Sandbox，不再展示首页菜单。

工具返回的是浏览器已加载启动页的确认，仿真随后还需初始化；不读取或虚构实时车辆数。当前只支持本机 30141 端口、无访问密码的单工作台演示。没有打开工作台、多窗口或加载超时都会返回错误。

官方 v0.3.49 的 `apps/game/src/lib.rs` 支持场景路径和 `--time` 参数，`abstutil/src/cli.rs` 将 URL 中的 `&` 分隔参数转换为命令行参数。无需修改 WASM。
